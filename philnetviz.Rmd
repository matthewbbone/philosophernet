...
nodes <- read_csv("nodes.csv") %>%
  ...
  mutate(title = paste0("<h3 style='color:Black;font-size:8px;'>", label, "</h3>"),
         ...
         Analytic = if_else(`Analytic philosophy` == 1 | 
                              `Analytic Philosophy` == 1 |
                              `Analytic` == 1 | 
                              `analytic philosophy` == 1 | 
                              `Analytic school`, 1, 0),
         Continental = if_else(`Continental` == 1 |
                                 `Continental Philosophy` == 1 |
                                 `Continental philosophy` == 1, 1, 0)) %>%
  select(-c(`Analytic philosophy`,`Analytic Philosophy`,`analytic philosophy`,`Analytic school`, 
